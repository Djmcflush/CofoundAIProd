...
from cofound_ai.llm.base import LLM


def get_llm(model='gpt-3.5-turbo-16k') -> LLM:
    if model.startswith('gpt'):
        from cofound_ai.llm.openai_llm import OpenaiLlm
        return OpenaiLlm(model=model)
    elif model.startswith('claude'):
        from cofound_ai.llm.anthropic_llm import AnthropicLlm
        return AnthropicLlm(model=model)
    elif "llama" in model:
        # Currently use Anyscale to support llama models
        from cofound_ai.llm.anyscale_llm import AnysacleLlm
