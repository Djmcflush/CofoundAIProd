...
from cofound_ai.audio.text_to_speech import get_text_to_speech
from cofound_ai.database.connection import get_db
from cofound_ai.models.interaction import Interaction
from cofound_ai.models.feedback import Feedback, FeedbackRequest
from cofound_ai.models.character import Character, CharacterRequest, \
    EditCharacterRequest, DeleteCharacterRequest, GeneratePromptRequest
from cofound_ai.models.quivr_info import QuivrInfo, UpdateQuivrInfoRequest
from cofound_ai.llm.system_prompt_generator import generate_system_prompt
...
def get_current_user(request: Request):
    """Helper function for auth with Clerk."""
    if os.getenv('USE_AUTH', ''):
        ...
        token = tokens[1]
        try:
            # Verify the token against the Clerk API.
            clerk_url = f"https://api.clerk.dev/v1/sessions/{token}"
            headers = {"Authorization": f"Bearer {os.getenv('CLERK_API_KEY')}"}
            response = requests.get(clerk_url, headers=headers)
...
async def characters(user=Depends(get_current_user)):
    ...
    from cofound_ai.character_catalog.catalog_manager import CatalogManager
